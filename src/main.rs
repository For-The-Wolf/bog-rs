...
async fn main() -> std::io::Result<()> {
    ...
    HttpServer::new(move || {
        ...
        App::new()
            ...
            .route(
                "/beta/single_player",
                web::get().to(web_server::single_player),
            )
            .route(
            ...
            .route(
                "/beta/create_room/{room_name}",
                web::post().to(web_server::create_lobby),
            )
            .route(
                "/beta/insert_player/{room_id}/{player_name}",
                web::post().to(web_server::insert_player),
            )
            .route(
                "/beta/lobby/{room_id}/{player_id}",
                web::get().to(web_server::lobby),
            )
            .route(
                "/beta/poll_lobby/{button_state}/{room_id}",
                web::get().to(web_server::poll_lobby),
            )
            .route(
                "/beta/multi_player/{room_id}/{player_id}",
                web::get().to(web_server::multi_player),
            )
            .route(
                "/beta/poll_game/{room_id}/{player_id}",
